...
class CreateInvoiceDownPaymentsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('invoice_down_payments', function (Blueprint $table) {
            ...
            $table->foreignIdFor(BankInternal::class)->nullable();
            $table->decimal('exchange_rate', 18, 2);
            $table->string('sale_order_model')->nullable();
            $table->bigInteger('sale_order_model_id')->nullable();
            $table->date('date');
            $table->date('due_date');
