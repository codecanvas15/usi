<?php

namespace App\Http\Controllers\Admin;

use App\Http\Controllers\Controller;
use App\Models\Asset;
use App\Models\AssetDocument;
use Carbon\Carbon;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Storage;
use Yajra\DataTables\Facades\DataTables;

class AssetDocumentController extends Controller
{
    /**
     * Instantiate a new controller instance.
     *
     * @return void
     */
    public function __construct()
    {
        $this->middleware("permission:view $this->view_folder", ['only' => ['index', 'show']]);
        $this->middleware("permission:create $this->view_folder", ['only' => ['create', 'store']]);
        $this->middleware("permission:edit $this->view_folder", ['only' => ['edit', 'update']]);
        $this->middleware("permission:delete $this->view_folder", ['only' => ['destroy']]);
    }

    /**
     * where the view will be displayed
     *
     * @var string
     */
    protected string $view_folder = 'asset-document';

    /**
     * search  value
     *
     * @var string|null
     */
    protected string|null $search = null;

    /**
     * Display a listing of the resource.
     *
     * @param  \Illuminate\Http\Request  $request
     * @return \Illuminate\Http\Response
     */
    public function index(Request $request)
    {
        if ($request->ajax()) {
            $data = AssetDocument::join('assets', 'assets.id', 'asset_documents.asset_id')
                ->select('asset_documents.*', 'assets.asset_name', 'assets.code as asset_code');

            // If Request Expired data
            $expiredData = [];
            if ($request->expired) {
                if ($data->count() > 0) {
                    foreach ($data->get() as $key => $value) {
                        if (Carbon::now()->gt(Carbon::parse($value->end_date)->subDays($value->due_date))) {
                            $expiredData[] = $value;
                        }
                    }
                }
            } else {
                $expiredData = $data;
            }

            return DataTables::of($expiredData)
                ->addIndexColumn()
                ->editColumn('asset_name', function ($row) {
                    $link = route('admin.asset.show', ['asset' => $row->id]);
                    if (auth()->user()->can("view asset")) {
                        return "<a href='$link' target='_blank'>$row->asset_name</a>";
                    } else {
                        return $row->asset_name;
                    }
                })
                ->editColumn('effective_date', fn ($row) => localDate($row->effective_date))
                ->editColumn('end_date', fn ($row) => localDate($row->end_date))
                ->addColumn('status', function ($row) {
                    $badge = '';
                    $expired_text = '';
                    $expired_in = Carbon::now()->diffForHumans($row->end_date, true);
                    if (Carbon::now()->gt($row->end_date)) {
                        $expired_text = "berakhir $expired_in yang lalu";
                        $badge .= '<span class="badge bg-danger">Expired</span>';
                    } else {
                        if (Carbon::now()->gt(Carbon::parse($row->end_date)->subDays($row->due_date))) {
                            $expired_text = "berakhir $expired_in lagi";
                        }
                    }

                    $badge .= "<span class='text-capitalize badge bg-dark'>$expired_text</span>";

                    return $badge;
                })
                ->editColumn('export', function ($row) {
                    $button = '';
                    $file = asset('storage/' . $row->file);
                    $button .= "<a href='$file' class='btn btn-sm btn-primary' target='_blank'><i class='fas fa-download'></i></a>";

                    $delete_route = route('admin.asset-document.destroy', ['asset_document' => $row->id]);
                    $datatable_id = '#asset_legality_table';

                    if (auth()->user()->can("edit $this->view_folder")) {
                        $button .= "<button class='btn btn-sm btn-warning' onclick='show_asset_edit_modal(`$row->id`)'><i class='fas fa-edit'></i></button>";
                    }

                    if (auth()->user()->can("delete $this->view_folder")) {
                        $button .= "<button class='btn btn-sm btn-danger' onclick='show_delete_confirmation(`$delete_route`, `$datatable_id`)'><i class='fas fa-trash'></i></button>";
                    }

                    return $button;
                })
                ->escapeColumns([])
                ->make(true);
        }

        return view('admin.' . $this->view_folder . '.index');
    }



    public function store(Request $request)
    {
        $this->validate($request, [
            'asset_id' => 'required|exists:assets,id',
            'name' => 'required|string',
            'transaction_date' => 'required',
            'effective_date' => 'required',
            'end_date' => 'required',
            'due_date' => 'required',
            'file' => 'required|file|mimes:pdf,jpg,jpeg,png|max:10048',
        ]);

        DB::beginTransaction();
        try {
            $asset_document = new AssetDocument();
            $asset_document->asset_id = $request->asset_id;
            $asset_document->name = $request->name;
            $asset_document->transaction_date = Carbon::parse($request->transaction_date);
            $asset_document->effective_date = Carbon::parse($request->effective_date);
            $asset_document->end_date = Carbon::parse($request->end_date);
            $asset_document->due_date = thousand_to_float($request->due_date);
            $asset_document->description = $request->description ?? '';
            $asset_document->file = $request->file('file')->store('asset-document', 'public');
            $asset_document->save();

            DB::commit();
            return $this->ResponseJsonMessageCRUD(true, 'create', 'berhasil menyimpan dokumen', null, 200);
        } catch (\Throwable $th) {
            //throw $th;
            DB::rollBack();
            return $this->ResponseJsonMessageCRUD(false, 'create', $th->getMessage(), null, 500);
        }
    }

    public function edit($id)
    {
        $asset_document = AssetDocument::findOrFail($id);

        return $this->ResponseJsonMessageCRUD(data: $asset_document);
    }

    public function update(Request $request, $id)
    {
        $this->validate($request, [
            'asset_id' => 'required|exists:assets,id',
            'name' => 'required|string',
            'transaction_date' => 'required',
            'effective_date' => 'required',
            'end_date' => 'required',
            'due_date' => 'required',
            'file' => 'nullable|file|mimes:pdf,jpg,jpeg,png|max:10048',
        ]);

        DB::beginTransaction();
        try {
            $asset_document = AssetDocument::findOrFail($id);
            $asset_document->name = $request->name;
            $asset_document->asset_id = $request->asset_id;
            $asset_document->transaction_date = Carbon::parse($request->transaction_date);
            $asset_document->effective_date = Carbon::parse($request->effective_date);
            $asset_document->end_date = Carbon::parse($request->end_date);
            $asset_document->due_date = thousand_to_float($request->due_date);;
            $asset_document->description = $request->description ?? '';
            if ($request->hasFile('file')) {
                Storage::delete($asset_document->file);
                $asset_document->file = $request->file('file')->store('asset-document', 'public');
            }
            $asset_document->save();

            DB::commit();
            return $this->ResponseJsonMessageCRUD(true, 'create', 'berhasil menyimpan dokumen', null, 200);
        } catch (\Throwable $th) {
            //throw $th;
            DB::rollBack();
            return $this->ResponseJsonMessageCRUD(false, 'create', $th->getMessage(), null, 500);
        }
    }

    public function destroy($id)
    {
        DB::beginTransaction();
        try {
            $asset_document = AssetDocument::findOrFail($id);
            Storage::delete($asset_document->file);
            $asset_document->delete();

            DB::commit();
            return $this->ResponseJsonMessageCRUD(true, 'delete', 'berhasil menghapus dokumen', null, 200);
        } catch (\Throwable $th) {
            DB::rollBack();
            return $this->ResponseJsonMessageCRUD(false, 'delete',  $th->getMessage(), null, 500);
            //throw $th;
        }
    }

    public function asset(Request $request)
    {
        if ($request->ajax()) {
            $data = Asset::select('assets.*');

            return DataTables::of($data)
                ->addIndexColumn()
                ->editColumn('asset_name', function ($row) {
                    $link = route('admin.asset.show', ['asset' => $row->id]);
                    if (auth()->user()->can("view asset")) {
                        return "<a href='$link' target='_blank'>$row->asset_name</a>";
                    } else {
                        return $row->asset_name;
                    }
                })
                ->editColumn('export', function ($row) {
                    $button = '';

                    if (auth()->user()->can("create $this->view_folder")) {
                        $button .= "<button class='btn btn-sm btn-info' onclick='show_create_modal_asset(`$row->id`)'><i class='fas fa-plus'></i></button>";
                    }

                    return $button;
                })
                ->escapeColumns([])
                ->make(true);
        }

        return view('admin.' . $this->view_folder . '.index');
    }
}
