...
<head>
    ...
    <title>Purchase Order Service Close - {{ $model->code }}</title>
    <style>
        body {
                        ...
                        <tr>
                            ...
                            <td class="p-0 valign-top text-uppercase">
                                @if ($model->type == 'purchase-request')
                                    @foreach ($model->purchaseOrderServiceDetails as $item)
                                        <p class="my-0">{{ $item->purchase_request->kode ?? '' }}</p>
                                    @endforeach
                                @else
                                    @foreach ($model->purchaseOrderServiceDetails as $item)
                                        <p class="my-0">{{ $item->sale_order_service->kode ?? '' }}</p>
                                    @endforeach
                                @endif
                            </td>
                        </tr>
            ...
            <tbody>
                ...
                @foreach ($model->purchaseOrderServiceDetails()->get() as $key => $purchaseOrderServiceDetail)
                    @foreach ($purchaseOrderServiceDetail->purchase_order_service_detail_items as $purchase_order_service_detail_item)
                        @php
                            $outstanding_qty = $purchase_order_service_detail_item->quantity - $purchase_order_service_detail_item->quantity_received;
                            $subtotal = $outstanding_qty * ($purchase_order_service_detail_item->price_display - $purchase_order_service_detail_item->discount);
                            $all_total += $subtotal;

                            $dpp += $subtotal;
                            $disc_total += $purchase_order_service_detail_item->discount * $outstanding_qty;
                        @endphp
                        <tr class="{{ $purchase_order_service_detail_item->quantity_received != $purchase_order_service_detail_item->quantity ? 'bg-warning' : '' }}">
                            <td style="vertical-align: top" width="5%" class="text-center">{{ $no++ }}.</td>
                            <td style="vertical-align: top" width="40%">{{ $purchase_order_service_detail_item?->item?->nama }}</td>
                            <td style="vertical-align: top" width="10%" class="text-end">{{ formatNumber($purchase_order_service_detail_item->quantity) }} {{ $purchase_order_service_detail_item->unit?->name }}</td>
                            <td style="vertical-align: top" width="10%" class="text-end">{{ formatNumber($purchase_order_service_detail_item->quantity_received) }} {{ $purchase_order_service_detail_item->unit?->name }}</td>
                            <td style="vertical-align: top" width="10%" class="text-end">{{ formatNumber($outstanding_qty) }} {{ $purchase_order_service_detail_item->unit?->name }}</td>
                            ...
                            <td style="vertical-align: top" class="text-end p-0">
                                <table>
                                    <tr>
                                        <td style="vertical-align: top" class="border-0"></td>
                                        <td style="vertical-align: top" class="text-right border-0">{{ formatNumber($purchase_order_service_detail_item->discount) }}</td>
                                    </tr>
                                </table>
                            </td>
                            <td style="vertical-align: top" class="text-end p-0">
