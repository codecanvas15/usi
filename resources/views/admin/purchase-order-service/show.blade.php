    ...
    @can('view purchase-service')
                ...
                <div class="box-body">
                    <div class="row justify-content-end">
                        <div class="col-md-6 align-self-center">
                            <h4 class="m-0">Detail Purchase Order Service</h4>
                            <h1 class="m-0">{{ $model->get_code }}</h1>
                        </div>
                        <div class="col-md-6 align-self-center">
                            <div class="row justify-content-end">
                                <div class="col-md-3 d-flex flex-column">
                                    <h5 class="text-center">{{ Str::headline('status_po_service') }}</h5>
                                    <div class="badge badge-lg badge-{{ purchase_service_status()[$model->status]['color'] }}">
                                        {{ Str::headline(purchase_service_status()[$model->status]['label']) }}
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="row">
            <div class="col-md-9">
                    ...
                    <x-slot name="table_content">
                        ...
                        <div class="row">
                            ...
                            @if ($model->is_spk)
                                <div class="col-md-3">
                                    <div class="form-group">
                                        <label for="">{{ Str::headline('nomor spk') }}</label>
                                        <p>{{ $model->spk_number }}</p>
                                    </div>
                                </div>
                            @endif
                            <div class="col-md-3">
                            ...
                            @if ($model->is_spk)
                                <div class="col-md-3">
                                    <div class="form-group">
                                        <label for="">{{ Str::headline('pic') }}</label>
                                        <p>{{ $model->pic }}</p>
                                    </div>
                                </div>
                            @endif
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <p>
                                    <div class="badge badge-lg badge-{{ purchase_service_status()[$model->status]['color'] }} mb-1">
                                        {{ purchase_service_status()[$model->status]['label'] }} -
                                        {{ purchase_service_status()[$model->status]['text'] }}
                                    </div>
                                    </p>
                                </div>
                            </div>
                ...
                <x-card-data-table title="Dari purchase order service">
                    <x-slot name="header_content">
                        ...
                        <x-table>
                            ...
                            <x-slot name="table_body">
                                @php
                                    $key = 0;
                                @endphp
                                @foreach ($model->purchaseOrderServiceDetails()->where('type', 'main')->get() as $key => $purchaseOrderServiceDetail)
                                    @foreach ($purchaseOrderServiceDetail->purchase_order_service_detail_items as $purchase_order_service_detail_item)
                                        <tr>
                                            <td>{{ $key += 1 }}</td>
                                            <td>{{ $purchase_order_service_detail_item?->item?->kode }} -
                                                {{ $purchase_order_service_detail_item?->item?->nama }}</td>
                                            <td class="text-end">{{ $model->currency->simbol }} {{ formatNumber($purchase_order_service_detail_item->price_before_discount) }}</td>
                                            <td class="text-end">{{ $model->currency->simbol }} {{ formatNumber($purchase_order_service_detail_item->discount) }}</td>
                                            <td class="text-end">{{ $model->currency->simbol }}
                                                {{ formatNumber($purchase_order_service_detail_item->price) }} /
                                                {{ $purchase_order_service_detail_item->unit?->name }}</td>
                                            <td>{{ formatNumber($purchase_order_service_detail_item->quantity) }}
                                                {{ $purchase_order_service_detail_item->unit->name }}</td>
                                            <td>{{ formatNumber($purchase_order_service_detail_item->quantity_received) }}
                                                {{ $purchase_order_service_detail_item->unit->name }}</td>
                                            <td class="text-end">{{ $model->currency->simbol }}
                                                {{ formatNumber($purchase_order_service_detail_item->sub_total) }}</td>
                                            <td>
                                                @foreach ($purchase_order_service_detail_item->purchase_order_service_detail_item_taxes as $purchase_order_service_detail_item_tax)
                                                    <p class="mb-2">{{ $purchase_order_service_detail_item_tax->tax->name }}
                                                        - {{ $purchase_order_service_detail_item_tax->value * 100 }}%</p>
                                                @endforeach
                                            </td>
                                            <td>
                                                @foreach ($purchase_order_service_detail_item->purchase_order_service_detail_item_taxes as $purchase_order_service_detail_item_tax)
                                                    <p class="mb-0 text-end">{{ $model->currency->simbol }}
                                                        {{ formatNumber($purchase_order_service_detail_item_tax->total) }}</p>
                                                @endforeach
                                            </td>
                                            <td class="text-end">{{ $model->currency->simbol }}
                                                {{ formatNumber($purchase_order_service_detail_item->total) }}</td>
                                            <td>
                                                <x-button color="info" icon="eye" fontawesome size="sm" dataToggle="modal" dataTarget="#purchase-order-service-item-{{ $purchase_order_service_detail_item->id }}-modal" />

                                                <x-modal title="Detail Data" id="purchase-order-service-item-{{ $purchase_order_service_detail_item->id }}-modal" headerColor="info" modalSize="1000">
                                                    <x-slot name="modal_body">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-md-3">
                                                                <div class="form-group">
                                                                    <label for="">{{ Str::headline('kode') }}</label>
                                                                    <p>{{ $purchaseOrderServiceDetail->purchase_request->kode }}
                                                                    </p>
                                                                </div>
                                                            </div>
                                                            ...
                                                            <div class="col-md-3">
                                                                <div class="form-group">
                                                                    <label for="">{{ Str::headline('tanggal') }}</label>
                                                                    <p>{{ localDate($purchaseOrderServiceDetail->purchase_request->tanggal) }}
                                                                    </p>
                                                                </div>
                                                            </div>
                                                            ...
                                                            <div class="col-md-3">
                                                                <div class="form-group">
                                                                    ...
                                                                    <p>{{ $purchaseOrderServiceDetail->purchase_request->division?->name }}
                                                                    </p>
                                                                </div>
                                                            </div>
                                                            ...
                                                            <div class="col-md-3">
                                                                <div class="form-group">
                                                                    ...
                                                                    <p>{{ $purchaseOrderServiceDetail->purchase_request->created_by_user?->email }}
                                                                        -
                                                                        {{ $purchaseOrderServiceDetail->purchase_request->created_by_user?->name }}
                                                                    </p>
                                                                </div>
                                                            </div>
                                                        ...
                                                        @php
                                                            $status_logs = $purchase_order_service_detail_item->logs_data['status_logs'] ?? [];
                                                            $activity_logs = $purchase_order_service_detail_item->logs_data['activity_logs'] ?? [];
                                                        @endphp
                ...
                @if ($model->purchaseOrderServiceDetails()->where('type', 'additional')->count())
                            ...
                            <x-table>
                                ...
                                <x-slot name="table_body">
                                    @php
                                        $key = 0;
                                    @endphp
                                    @foreach ($model->purchaseOrderServiceDetails()->where('type', 'additional')->get() as $purchaseOrderServiceDetail)
                                        @foreach ($purchaseOrderServiceDetail->purchase_order_service_detail_items as $purchase_order_service_detail_item)
                                            <tr>
                                                <td>{{ $key += 1 }}</td>
                                                <td>{{ $purchase_order_service_detail_item?->item?->kode }} -
                                                ...
                                                <td>
                                                    @foreach ($purchase_order_service_detail_item->purchase_order_service_detail_item_taxes as $purchase_order_service_detail_item_tax)
                                                        <p class="mb-2">
                                                            {{ $purchase_order_service_detail_item_tax->tax->name }} -
                                                            {{ $purchase_order_service_detail_item_tax->value * 100 }}%</p>
                                                    @endforeach
                                                </td>
                                                <td>
                                                    @foreach ($purchase_order_service_detail_item->purchase_order_service_detail_item_taxes as $purchase_order_service_detail_item_tax)
                                                        <p class="mb-0 text-end">{{ $model->currency->simbol }}
                                                            {{ formatNumber($purchase_order_service_detail_item_tax->total) }}
                                                        </p>
                                                    @endforeach
                                                </td>
                                                <td class="text-end">{{ $model->currency->simbol }}
                                                    {{ formatNumber($purchase_order_service_detail_item->total) }}</td>
                                                <td>
                                                    <div class="badge badge-lg badge-{{ purchase_service_status()[$purchase_order_service_detail_item->status]['color'] }}">
                                                        {{ purchase_service_status()[$purchase_order_service_detail_item->status]['label'] }}
                                                        -
                                                        {{ purchase_service_status()[$purchase_order_service_detail_item->status]['text'] }}
                                                    </div>
                                                </td>
                                                <td>
                                                    <x-button color="info" icon="eye" fontawesome size="sm" dataToggle="modal" dataTarget="#purchase-order-service-item-{{ $purchase_order_service_detail_item->id }}-modal" />
            ...
            <div class="col-md-3">
                ...
                <x-card-data-table>
                    <x-slot name="table_content">
                        @if ($model->is_spk)
                            <x-button-auth-print type="purchase_order_service" model="{{ \App\Models\PurchaseOrderService::class }}" did="{{ $model->id }}" href="{{ route($main . '.export-pdf', ['id' => encryptId($model->id), 'type' => 'spk']) }}" code="{{ $model->code }}" label="Export SPK" link="" symbol="&" />
                        @else
                            <x-button-auth-print type="purchase_order_service" model="{{ \App\Models\PurchaseOrderService::class }}" did="{{ $model->id }}" href="{{ route($main . '.export-pdf', ['id' => encryptId($model->id)]) }}" code="{{ $model->code }}" label="Export" link="" />
                        @endif
                    </x-slot>
                </x-card-data-table>
...
@section('js')
    ...
    <script>
        ...
        $('#history-button').on('click', function() {
            $.ajax({
                url: `{{ route('admin.purchase-order-service.history', $model->id) }}`,
                success: function({
                    data
                }) {
                    $('#history-list').html('');
        ...
        get_request_print_approval(`App\\Models\\PurchaseOrderService`, '{{ $model->id }}', 'purchase_order_service');
    </script>
@endsection
