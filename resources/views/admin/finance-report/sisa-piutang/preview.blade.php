                ...
                <table class="table table-bordered table-striped mt-10">
                    ...
                    <tbody>
                        @forelse ($data as $key => $d)
                            <tr>
                                ...
                                <td class="text-center">{{ $d->overdue ? formatNumber($d->overdue) : '-' }}</td>
                                <td class="text-end">{{ formatNumber($d->total) }}</td>
                                <td class="text-end">{{ formatNumber($d->paid_amount) }}</td>
                                <td class="text-end">{{ formatNumber($d->outstanding_amount) }}</td>
                                <td class="text-end">{{ formatNumber($d->exchange_rate) }}</td>
                                <td class="text-end">{{ formatNumber($d->total_exchanged) }}</td>
                                <td class="text-end">{{ formatNumber($d->paid_amount_exchanged) }}</td>
                                <td class="text-end">{{ formatNumber($d->acumulated_exchange_rate_gap) }}</td>
                                <td class="text-end">{{ formatNumber($d->outstanding_amount_exchanged) }}</td>
                            </tr>
                        @empty
                            <tr>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <th class="text-end">{{ formatNumber($data->sum('total_exchanged')) }}</th>
                            <th class="text-end">{{ formatNumber($data->sum('paid_amount_exchanged')) }}</th>
                            <th class="text-end"></th>
                            <th class="text-end">{{ formatNumber($data->sum('outstanding_amount_exchanged')) }}</th>
                        </tr>
                    </tfoot>
                </table>
            </div>
