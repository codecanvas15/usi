...
<head>
    ...
    <title>PDK LPB {{ $model->code }}</title>
    <style>
        body {
...
<body>
    ...
    <div class="container" style="color: black">
                ...
                @if ($model->item == 'lpb')
                    @foreach ($model->fund_submission_supplier_details as $item)
                        ...
                        <tr>
                            ...
                            <td class="small-font">{!! implode('<br>', $item->lpb_reference ?? []) !!}</td>
                            <td class="small-font">{{ $item->supplier_invoice_parent->reference ?? ($item->supplier_invoice_parent->code ?? '') }}</td>
                            <td class="small-font">{{ $item->note ?? '' }}</td>
                            <td class="small-font" style="white-space: nowrap;">{{ localDate($item->supplier_invoice_parent->due_date) }}</td>
                    ...
                    @foreach ($model->fund_submission_customers ?? [] as $item)
                        ...
                        <tr>
                            <td class="small-font"></td>
                            <td class="small-font"></td>
                            <td class="small-font">{{ $item->invoice_parent->code }}</td>
                            <td class="small-font">{{ $item->note ?? '' }}</td>
                            <td class="small-font">{{ localDate($item->invoice_parent->due_date) }}</td>
                            <td class="small-font p-0">
                                <table class="table">
                                    <tbody>
                                        <tr>
                        ...
                        @endif
                    @endforeach
                    @foreach ($model->fund_submission_purchase_returns ?? [] as $item)
                        <tr>
                            <td class="small-font"></td>
                            <td class="small-font">{{ $item->purchase_return->item_receiving_report ? $item->purchase_return->item_receiving_report?->kode . ' - ' . $item->purchase_return->code : $item->purchase_return->code }}</td>
                            <td class="small-font"></td>
                            <td class="small-font">{{ $item->purchase_return->code }}</td>
                            <td class="small-font">{{ localDate($item->purchase_return->date) }}</td>
                            <td class="small-font p-0">
                                <table class="table">
                                    <tbody>
                                        <tr>
        ...
        @if (count($cash_advance_payments) > 0)
            <h2 class="text-uppercase font-small-3 mt-2">Riwayat Uang Muka</h2>
            <table class="table table-bordered mb-2">
                <thead>
                    <tr>
                        <th class="small-font">TANGGAL</th>
                        <th class="small-font">KET.</th>
                        <th class="small-font">JUMLAH</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($cash_advance_payments as $cash_advance_payment)
                        <tr>
                            <td class="small-font text-center">{{ localDate($cash_advance_payment->date) }}</td>
                            <td class="small-font">{{ $cash_advance_payment->reference }}</td>
                            <td class="small-font p-0">
                                <table class="table">
                                    <tbody>
                                        <tr>
                                            <td class="border-0">{{ $cash_advance_payment->currency->simbol }}</td>
                                            <td class="border-0 text-right">{{ floatDotFormat($cash_advance_payment->cash_advance_cash_advance->debit ?? 0) }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </td>
        ...
        @if ($model->item == 'lpb' && $is_payment_history)
            ...
            @foreach ($model->fund_submission_supplier_details ?? [] as $fund_submission_supplier_detail)
                @php
                    $show_payment_information_general = $fund_submission_supplier_detail->supplier_invoice_parent->type == 'general' && $fund_submission_supplier_detail->payment_informations->count() > 0;
                    $show_payment_information_item_receiving_report =
                        $fund_submission_supplier_detail->supplier_invoice_parent->type != 'general' &&
                        $fund_submission_supplier_detail->supplier_invoice_parent->reference_model->detail->count() > 0 &&
                        $fund_submission_supplier_detail->supplier_invoice_parent->reference_model->detail->sum(function ($detail) {
                            return $detail->item_receiving_report->payment_informations->count() > 0;
                        });
                @endphp

                @if ($show_payment_information_general || $show_payment_information_item_receiving_report)
                    <table class="table table-bordered mb-2">
                        <tbody>
                            <tr class="bg-dark">
