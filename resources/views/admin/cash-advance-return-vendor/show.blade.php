            ...
            <div class="col-md-9">
                <x-card-data-table title='{{ "Detail $title" }}'>
                    <x-slot name="table_content">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <p>
                                    <div class="badge badge-lg badge-{{ cash_advance_return()[$model->status]['color'] }}">
                        ...
                        @if (count($cash_advance_return_invoice_details))
                            <div class="mt-20">
                                <h4>{{ Str::headline('detail lpb') }}</h4>
                                <x-table theadColor='danger'>
                                    <x-slot name="table_head">
                                        <th>#</th>
                                    ...
                                    <x-slot name="table_body">
                                        @foreach ($cash_advance_return_invoice_details as $item)
                                            <tr>
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{ $item->item_receiving_report->kode }}</td>
                                                <td class="text-end">{{ $model->invoice_currency->simbol }}
                                                    {{ formatNumber($item->outstanding) }}</td>
                                                <td class="text-end">{{ $model->invoice_currency->simbol }}
                                                    {{ formatNumber($item->amount) }}</td>
                                            </tr>
                                        @endforeach
                                    ...
                                    <x-slot name="table_foot">
                                        <tr>
                                            ...
                                            <td class="text-end">{{ $model->invoice_currency->simbol }}
                                                {{ formatNumber($cash_advance_return_invoice_details->sum('outstanding')) }}
                                            </td>
                                            <td class="text-end">{{ $model->invoice_currency->simbol }}
                                                {{ formatNumber($cash_advance_return_invoice_details->sum('amount')) }}</td>
                                        </tr>
                                    </x-slot>
                                </x-table>
                            </div>
                    ...
                    <x-slot name="table_content">
                        <div class="d-flex justify-content-end gap-3">
                            ...
                            @if (in_array($model->status, ['pending', 'revert']) && $model->check_available_date)
                                @can("edit $main")
                                    <x-button color='warning' fontawesome icon="edit" class="w-auto" size="sm" link="{{ route('admin.cash-advance-return-vendor.edit', $model) }}" />
                                @endcan
                            @endif
                            <x-button color="secondary mr-2" label="back" link="{{ url()->previous() }}" />
                        </div>
                    </x-slot>
                </x-card-data-table>
    ...
    <script>
        ...
        const displayInvoicePaymentInformation = () => {
            ...
            $.ajax({
                ...
                data: {
                    _token: token,
                    supplier_invoice_ids: JSON.parse('{!! $supplier_invoice_parent_ids !!}'),
                    date: '{{ $model->date }}'
                },
