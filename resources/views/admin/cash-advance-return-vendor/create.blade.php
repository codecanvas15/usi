...
@section('content')
    @can("create $main")
        <form action="{{ route("admin.$route.store") }}" method="post" id="form-create-cash-advance-data">
            <x-card-data-table title='{{ "Tambah $title" }}'>
                ...
                <x-slot name="table_content">
                    ...
                    <div class="row">
                        <div class="col-md-3">
                            <div class="form-group">
                                <x-select name="reference_id" label="vendor" id="vendor-select" required autofocus>
                    ...
                    <div class="row mt-20">
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                <x-select name="supplier_invoice_currency_id" label="mata uang purchase invoice" id="supplier-invoice-select-currency">
                                    <option value="{{ get_local_currency()->id }}" selected>{{ get_local_currency()->nama }}</option>
                                </x-select>
                            </div>
                        </div>
            ...
            <x-card-data-table title="Tambah Purchase Invoice">
                <x-slot name="table_content">
                    <div class="d-flex flex-column">
                        <div>
                            <x-button color="primary" class="float-end" id="add-invoice-modal-btn" label="Tambah Purchase Invoice" icon="plus" />
                        </div>
                        <div class="mt-10">
        ...
        <x-modal title="Tambah Uang muka" headerColor="primary" id="add-cash-advance" modalSize="900">
            <x-slot name="modal_body">
                <x-table theadColor="danger" id="cash-advance-payment">
                    <x-slot name="table_head">
                        <th>#</th>
                        <th>{{ Str::headline('Tanggal') }}</th>
                        <th>{{ Str::headline('Nomor transaksi') }}</th>
        ...
        <x-modal title="Edit Purchase Invoice" headerColor="primary" id="edit-invoice-modal" modalSize="1000">
            <x-slot name="modal_body">
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                    ...
                    <div class="col-md-12">
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    <x-input type="text" name="amount-to-return" label="amount to return" class="commas-form" id="edit-invoice-amount-to-return" disabled required />
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
            ...
            $(document).ready(function() {
                ...
                const firstCardFuntion = () => {
                    ...
                    const currencyTrigger = () => {
                        ...
                        const resetCurrencyValue = () => {
                            ...
                            $('#supplier-invoice-select-currency').val(parent_currency.id)

                            $('#currency-select').html(`<option value="{{ get_local_currency()->id }}" selected>{{ get_local_currency()->nama }}</option>`);
                            $('#supplier-invoice-select-currency').html(`<option value="{{ get_local_currency()->id }}" selected>{{ get_local_currency()->nama }}</option>`);
                            ...
                            initSelect2Search('supplier-invoice-select-currency', "{{ route('admin.select.currency') }}", {
                                id: "id",
                                text: "kode,nama,negara"
                            });
                        ...
                        $('#supplier-invoice-select-currency').change(function(e) {
                            e.preventDefault();
                ...
                const secondCardFunction = () => {
                    ...
                    const initializeSecondCard = () => {
                        getData();
                        triggerModalAddCashAdvancePayment();
                    };

                    const getData = () => {
                        $.ajax({
                            type: "POST",
                            url: `{{ route('admin.cash-advance-return-vendor.cash-advance-payments') }}`,
                            data: {
                                vendor_id: VENDOR_ID,
                            ...
                            success: function({
                                ...
                                CASH_ADVANCE_PAYMENT_LIST = data;
                                SELECTED_CASH_ADVANCE_PAYMENT_LIST_DATA = [];
                                displayCashAdvancePaymentData()
                            }
                        });
                    };

                    const displayCashAdvancePaymentData = () => {
                        $('#cash-advance-payment tbody').html('');
                        ...
                        CASH_ADVANCE_PAYMENT_LIST.map((cash_advance_payment, cash_advance_payment_index) => {
                            ...
                            cash_advance_payment_details.map((cash_advance_payment_detail, cash_advance_payment_detail_index) => {
                                if (cash_advance_payment_detail.type == 'cash_advance') {
                                    let {
                                        coa,
                                        debit
                                    } = cash_advance_payment_detail;

                                    let cash_advance_tax = cash_advance_payment_details.find(detail => detail.type == 'tax');
                                    if (cash_advance_tax) {
                                        debit += cash_advance_tax.debit;
                                    }

                                    $('#cash-advance-payment tbody').append(`
                                    <tr>
                                        ...
                                        <td>${currency.simbol} ${formatRupiahWithDecimal(debit)}</td>
                                        <td>
                                            <span class="d-flex">
                                                <span class="me-5">${currency.simbol}</span>
                                                <span id="cash-advance-payment-outstanding-amount-${cash_advance_payment_index}">${formatRupiahWithDecimal(debit - cash_advance_payment_detail.cash_advance_return_total)}</span>
                                            </span>
                                        </td>
                                        <td>
                                            <div class="d-flex align-items-center gap-2">
                                                ...
                                                <x-input-checkbox label="-" name="check" id="checkbox-cash-advance-${cash_advance_payment_index}" hideAsterix />
                                            </div>
                                        </td>
                                    </tr>
                                `);
                                    ...
                                    SELECTED_CASH_ADVANCE_PAYMENT_LIST_DATA[cash_advance_payment_index] = {
                                        selected: false,
                                        amount: 0,
                                        balance: 0,
                                        outstanding_amount: debit - cash_advance_payment_detail.cash_advance_return_total,
                                        data: cash_advance_payment,
                                        purchase: cash_advance_payment.purchase
                                    };

                                    $(`#edit-cash-advance-data-${cash_advance_payment_index}`).click(function(e) {
                                        e.preventDefault();
                                        editCashAdvancePayment(cash_advance_payment_index);
                                    });

                                    $(`#checkbox-cash-advance-${cash_advance_payment_index}`).click(function(e) {
                                        SELECTED_CASH_ADVANCE_PAYMENT_LIST_DATA[cash_advance_payment_index].selected = this.checked;
                                    });
                                }
                            })
                        });
                    ...
                    const triggerModalAddCashAdvancePayment = () => {
                        ...
                        $('#save-cash-advance-data').click(function(e) {
                            ...
                            displayCashAdvanceResume()
                            calculateTransactionResume()
                        });
                    };

                    const editCashAdvancePayment = (cash_advance_index) => {
                        $('#edit-cash-advance').modal('show');

                        let single_data = SELECTED_CASH_ADVANCE_PAYMENT_LIST_DATA[cash_advance_index];
                        ...
                        cash_advance_payment_details.map((cash_advance_payment_detail, cash_advance_payment_detail_index) => {
                            if (cash_advance_payment_detail.type == 'cash_advance') {
                                ...
                                $('#cash-advance-date').val(localDate(date));
                                $('#cash-advance-trasaction-no').val(bank_code_mutation ?? code);
                                $('#cash-advance-account-code').val(`${coa.account_code} - ${coa.name}`);
                                $('#cash-advance-currency').val(`${currency.nama}`);
                                $('#cash-advance-rate').val(formatRupiahWithDecimal(exchange_rate));
                                $('#cash-advance-total-amount').val(formatRupiahWithDecimal(debit));
                                $('#cash-advance-returned-amount').val(formatRupiahWithDecimal(cash_advance_return_total));
                                $('#cash-advance-outstanding-amount').val(formatRupiahWithDecimal(debit - cash_advance_return_total));
                                ...
                                $('#cash-advance-description').val(note);

                                single_data.outstanding_amount = debit - cash_advance_return_total;
                            }
                        })
                        ...
                        $('#save-edit-cash-advance-data').click(function(e) {
                            ...
                            $(`#cash-advance-payment-amount-to-return-html-${cash_advance_index}`).html(formatRupiahWithDecimal(single_data.amount));
                            $(`#cash-advance-payment-outstanding-amount-${cash_advance_index}`).html(formatRupiahWithDecimal(single_data.outstanding_amount));
                            $(`#cash-advance-payment-balance-${cash_advance_index}`).html(formatRupiahWithDecimal(single_data.balance));
                    ...
                    const displayCashAdvanceResume = () => {
                        $('#cash-advance-payment-resume tbody').html('');

                        SELECTED_CASH_ADVANCE_PAYMENT_LIST_DATA.map((cash_advance_payment, cash_advance_payment_index) => {
                            if (cash_advance_payment.selected) {
                                let {
                                    data,
                                    amount,
                                    balance,
                                    ...
                                    if (cash_advance_payment_detail.type == 'cash_advance') {
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <span class="d-flex">
                                                    <span class="me-5">${currency.simbol}</span>
                                                    <span id="cash-advance-payment-amount-to-return-html-${cash_advance_payment_index}">${formatRupiahWithDecimal(amount)}</span>
                                                </span>
                                            </td>
                                            <td>
                                                <span class="d-flex">
                                                    <span class="me-5">${data.currency.simbol}</span>
                                                    <span id="cash-advance-payment-balance-${cash_advance_payment_index}">${formatRupiahWithDecimal(balance)}</span>
                                                </span>
                                            </td>
                                            <td>
                                                <input type="hidden" name="cash_advance_payments[]" value="${data.id}" />
                                                <input type="hidden" name="cash_advance_payment_amount_to_returns[]" value="${amount}" />
                                            </td>
                                        </tr>
                                    `);
                                    }
                ...
                const thirdCardFunction = () => {
                    ...
                    const getValueOfGap = (invoice_index) => {
                        ...
                        let this_invoice_currency = SELECTED_SUPPLIER_INVOICE_LIST_DATA[invoice_index].data.currency;
                        let this_data = SELECTED_SUPPLIER_INVOICE_LIST_DATA[invoice_index].data;
                    ...
                    const displayDataModal = () => {
                        ...
                        SUPPLIER_INVOICE_LIST.map((invoice, invoice_index) => {
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="d-flex">
                                        <span>
                                            ${invoice.currency.simbol} ${formatRupiahWithDecimal(invoice.outstanding_amount)}
                                        </span>
                                    </span>
                                </td>
                                <td>
                        ...
                        `);
                            ...
                            SELECTED_SUPPLIER_INVOICE_LIST_DATA[invoice_index] = {
                                selected: false,
                                data: invoice,
                                amount_to_return: 0,
                                amount_to_return_list: [],
                                description: null,
                            }
                            ...
                            $(`#check-cash-advance-return-invoice-${invoice_index}`).click(function(e) {
                                SELECTED_SUPPLIER_INVOICE_LIST_DATA[invoice_index].selected = this.checked ? true : false;
                            });
                        })
                    ...
                    const displayResumeData = () => {
                        ...
                        SELECTED_SUPPLIER_INVOICE_LIST_DATA.map((invoice, invoice_index) => {
                            if (invoice.selected) {
                                let html_amount_to_return_item_receiving_report = '';

                                let loop_index = 0;
                                invoice.data.supplier_invoice.map((supplier_invoice, supplier_invoice_index) => {
                                    supplier_invoice.detail.map((detail, detail_index) => {
                                        html_amount_to_return_item_receiving_report += `
                                            <input type="hidden" name="item_receiving_report_ids[${invoice_index}][${loop_index}]" value="${detail.item_receiving_report_id}" />
                                            <input type="hidden" name="item_receiving_report_amount_to_return[${invoice_index}][${loop_index}]" value="${detail.amount_to_return}" />
                                        `;
                                ...
                                <tr>
                                    <td>${invoice_index + 1}</td>
                                    <td>${localDate(invoice.data.date)}</td>
                                    <td>${invoice.data.code}</td>
                                    <td>${invoice.data.currency.nama} / ${formatRupiahWithDecimal(invoice.data.exchange_rate)}</td>
                                    <td>${invoice.data.currency.simbol} ${formatRupiahWithDecimal(invoice.data.total)}</td>
                                    <td>${invoice.data.currency.simbol} ${formatRupiahWithDecimal(invoice.data.outstanding_amount)}</td>
                                    <td>${invoice.data.currency.simbol} ${formatRupiahWithDecimal(invoice.amount_to_return)}</td>
                                    <td>${invoice.data.currency.simbol} ${formatRupiahWithDecimal(invoice.data.outstanding_amount - invoice.amount_to_return)}</td>
                                    <td>
                                        <input type="hidden" name="invoice_ids[]" value="${invoice.data.id}" />
                                        <input type="hidden" name="invoice_amount_to_returns[]" value="${invoice.amount_to_return}" />
                                        <input type="hidden" name="invoice_descriptions[]" value="${invoice.description}" />
                                        ${html_amount_to_return_item_receiving_report}
                                    </td>
                                </tr>
                            `);
                    ...
                    const handleAddModal = () => {
                        $.ajax({
                            type: "POST",
                            url: `{{ route('admin.cash-advance-return-vendor.unpaid-full-supplier-invoices') }}`,
                            data: {
                                vendor_id: VENDOR_ID,
                                currency_id: $('#supplier-invoice-select-currency').val(),
                    ...
                    const displayInvoicePaymentInformation = () => {
                        ...
                        $.ajax({
                            ...
                            data: {
                                _token: token,
                                supplier_invoice_ids: SELECTED_SUPPLIER_INVOICE_LIST_DATA.map((data) => {
                                    if (data.selected) {
                                        return data.data.id
                                    }
                                }),
                    ...
                    const editInvoice = (invoice_index) => {
                        ...
                        let latest_amount_to_return = amount_to_return;
                        ...
                        data.supplier_invoice.map((supplier_invoice, supplier_invoice_index) => {
                            supplier_invoice.detail.map((detail, detail_index) => {

                                if (!detail.amount_to_return) {
                                    detail.amount_to_return = 0;
                                }

                                var selected_cap = [];
                                SELECTED_CASH_ADVANCE_PAYMENT_LIST_DATA.filter((selected_data) => {
                                    if (selected_data.selected) {
                                        selected_cap.push({
                                            id: selected_data.purchase.model_id,
                                            model: selected_data.purchase.model_reference,
                                        })
                                    }
                                });
                                ...
                                $('#supplier-invoice-table tbody').append(`
                                    <tr>
                                        <td>${loop_index + 1}</td>
                                        <td>${detail.item_receiving_report?.kode ?? "undefined"}</td>
                                        <td>${formatRupiahWithDecimal(detail.item_receiving_report?.outstanding) ?? "undefined"}</td>
                                        <td>
                                            <div class="form-group">
                                               <x-input type="text" value="${formatRupiahWithDecimal(detail.amount_to_return ?? 0)}" id="amount-to-return-item-receiving-report-${loop_index}" label="-" class="commas-form" hideAsterix placeholder="Amount to return" required />
                                            </div>
                                        </td>
                                    </tr>
                                `);

                                let single_index = loop_index;
                                $(`#amount-to-return-item-receiving-report-${loop_index}`).keyup(function(e) {

                                    if (thousandToFloat($(this).val()) > detail.item_receiving_report.outstanding) {
                                        alert("Melebihi jumlah pengembalian yang tersedia");
                                        this.value = formatRupiahWithDecimal(detail.amount_to_return);
                                    } else {
                                        detail.amount_to_return = thousandToFloat($(this).val());
                                    }

                                    amount_to_return_list[single_index] = detail.amount_to_return;

                                    latest_amount_to_return = amount_to_return_list.reduce((total, data) => {
                                        return total += data;
                                    }, 0);

                                    $('#edit-invoice-amount-to-return').val(formatRupiahWithDecimal(latest_amount_to_return));
                                    $('#edit-invoice-amount-to-return').trigger('keyup');
                                });
                        ...
                        $('#save-edit-invoice-data').click(function(e) {
                            ...
                            SELECTED_SUPPLIER_INVOICE_LIST_DATA[invoice_index].description = description;
                            SELECTED_SUPPLIER_INVOICE_LIST_DATA[invoice_index].amount_to_return = amount_to_return;
                        ...
                        $('#edit-invoice-modal').on('hide.bs.modal', function() {
                            ...
                            data.supplier_invoice.map((supplier_invoice, supplier_invoice_index) => {
                                supplier_invoice.detail.map((detail, detail_index) => {
                                    $(`#amount-to-return-item-receiving-report-${loop_index}`).unbind('keyup');
                                });
                            });
                    ...
                    const addOtherTrasaction = (transaction_index) => {
                        ...
                        $('#save-other-trasaction-data').click(function(e) {
                            ...
                            if (
                                ...
                                alert("Harap isi data coa dan deskripsi terlebih dahulu");
                                return;
                            }

                            if (!$('#other-transaction-amount').val()) {
                                alert("Harap isi data amount");
                                return;
                            }
                    ...
                    `);
                        ...
                        $('#save-other-trasaction-edit').click(function(e) {
                            ...
                            if (
                                ...
                                alert("Harap isi data coa dan deskripsi");
                                return;
                            }
                ...
                const calculateTransactionResume = () => {
                    ...
                    SELECTED_CASH_ADVANCE_PAYMENT_LIST_DATA.forEach((data) => {
                        if (data.selected) {
                            total_cash_advance += data.amount;
                        }
                    });

                    SELECTED_SUPPLIER_INVOICE_LIST_DATA.forEach((data) => {
                        if (data.selected) {
                            total_invoice += data.amount_to_return;
                        }
                    });
                    ...
                    $('#gap_total_display').text(formatRupiahWithDecimal(total_cash_advance - total_invoice - total_other_transaction))
                }
