...
<head>
    ...
    <title>Pengembalian Uang Muka Vendor {{ $model->code }}</title>
    <style>
        body {
...
<body>
    ...
    <div class="container" style="color: black">
        <div class="text-center">
            <h3>PENGEMBALIAN UANG MUKA VENDOR</h3>
        </div>
        <div class="mt-2">
            <table width="100%">
                <tr>
                    <td class="small-font" width="10%"><b>Kode Transaksi</b></td>
                    <td class="small-font" width="2%">:</td>
                    <td class="small-font" width="25%">{{ $model->code }}</td>
                    <td class="small-font" width="10%"></td>
                    <td class="small-font text-right" width="10%"><b>Tanggal</b></td>
                    <td class="small-font" width="2%">:</td>
                ...
                <tr>
                    ...
                    <td class="small-font"></td>
                    <td class="small-font text-right"><b>Vendor</b></td>
                    <td class="small-font">:</td>
                                ...
                                <tbody>
                                    <tr class="border-0">
                                        <td class="p-0 border-0">
                                            {{ $cashAdvanceReturnDetail->reference->bank_code_mutation ?? $cashAdvanceReturnDetail->reference->code }}
                                        </td>
                                        <td class="p-0 border-0">
                                            {{ optional(($cashAdvanceReturnDetail->reference_model)::find($cashAdvanceReturnDetail->reference_id))->fund_submission->code ?? '-' }}
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                        ...
                        <td class="font-xsmall-3">
                            <table class="table border-0" style="border-collapse: collapse">
                                <tbody>
                                    <tr class="border-0">
                                        <td class="p-0 border-0">
                                            {{ $cashAdvanceReturnInvoices->transaction_code }}
                                        </td>
                                        <td class="p-0 border-0">
                                            @foreach ($cashAdvanceReturnInvoices->reference->reference_model->detail ?? [] as $detail)
                                                <p class="m-0">{{ $detail->item_receiving_report->kode }}</p>
                                            @endforeach
                                        </td>
                                    </tr>
                                </tbody>
                        ...
                        <td class="small-font">
                            <table class="border-0">
                                <tbody>
                                    <tr class="border-0">
                                        <td class="border-0 text-left">{{ $model->currency->simbol }}</td>
                                        <td class="border-0 text-right">{{ formatNumber($cashAdvancedReturnTransaction->debit - $cashAdvancedReturnTransaction->credit) }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </td>
                    ...
                    <td class="small-font">
                        <table class="border-0">
                            <tbody>
                                <tr class="border-0">
                                    <th class="border-0 text-left">{{ $model->currency->simbol }}</th>
                                    <th class="border-0 text-right">{{ formatNumber($model->cashAdvancedReturnTransactions->sum('debit') - $model->cashAdvancedReturnTransactions->sum('credit')) }}</th>
                                </tr>
                            </tbody>
                        </table>
                    </td>
