...
@section('content')
    <form action="{{ route("admin.$main.update", $model) }}" method="post" id="form_purchase_request" enctype="multipart/form-data">
        <x-card-data-table title="{{ 'tambah ' . $main }}">
            ...
            <x-slot name="table_content">
                ...
                <div class="row mt-20">
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            <x-input class="datepicker-input" id="tanggal" name="date" value="{{ \Carbon\Carbon::today()->format('d-m-Y') }}" onchange="checkClosingPeriod($(this))" autofucus required value="{{ localDate($model->date) }}" />
                        </div>
                    </div>
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            <x-select name="sh_number_id" id="sh_number_id" label="sh no." required>
                                <option value="{{ $model->sh_number->id }}">{{ $model->sh_number->kode }}</option>
                            </x-select>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-3">
                            <x-select name="item_id" id="item_id" label="item" required>
                                <option value="{{ $model->purchase_request_trading_details()->first()->item->id }}">{{ $model->purchase_request_trading_details()->first()->item->nama }}</option>
                            </x-select>
                        </div>
                        <div class="col-md-3">
                            <x-input name="qty" id="qty" label="quantity" required class="commas-form text-end" value="{{ formatNumber($model->purchase_request_trading_details()->first()->qty) }}" />
                        </div>
                    </div>
                    <div class="row mt-30">
                        <div class="col-md-6">
                            <x-text-area name="note" label="keterangan" id="note" cols="30" rows="10" required>{{ $model->note }}</x-text-area>
                        </div>
                    </div>
                </div>
