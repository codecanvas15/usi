    ...
    @can("view $main")
        <div class="row">
            <div class="col-md-7">
                ...
                <x-card-data-table title="{{ 'amortisasi' }}">
                    <x-slot name="header_content">
                        ...
                        <x-table theadColor='danger'>
                            ...
                            <x-slot name="table_body">
                                @foreach ($model->amortizations as $amortization)
                                    <tr>
                                        <td>
                                            {{ localDate($amortization->date) }}
                                        </td>
                                        <td class="text-end">
                                            {{ formatNumber($amortization->amount) }}
                                        </td>
                                        <td>
                                            {{ $amortization->note }}
                                        </td>
                            ...
                            <x-slot name="table_foot">
                                ...
                                <tr>
                                    <th>{{ Str::headline('JUMLAH AMORTISASI') }}</th>
                                    <th class="text-end">{{ formatNumber($model->amortizations->sum('amount')) }}</th>
                                    <th></th>
                                </tr>
                                <tr>
            ...
            <div class="col-md-5">
                <x-card-data-table title="{{ 'Action' }}">
                    ...
                    <x-slot name="table_content">
                        ...
                        @if ($model->status != 'cancel' && $model->status != 'inactive' && $model->amortizations->count() == 0)
                            @can('delete lease')
                                <x-button color="danger" icon="times" fontawesome label="Batalkan Biaya dibayar dimuka" size="sm" dataToggle="modal" dataTarget="#cancel-modal" />
                                <x-modal title="batalkan biaya dibayar dimuka" id="cancel-modal" headerColor="success">
                    ...
                    <x-slot name="table_content">
                        ...
                        <x-table theadColor='danger'>
                            ...
                            <x-slot name="table_body">
                                ...
                                <tr>
                                    <th>{{ Str::headline('durasi') }}</th>
                                    <td>{{ formatNumber($model->month_duration) }} bulan</td>
                                </tr>
                                <tr>
                                    <th>{{ Str::headline('nilai amortisasi /bulan') }}</th>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $model->acumulated_depreciation_coa?->account_code }} - {{ $model->acumulated_depreciation_coa?->name }}</td>
                                </tr>
                                <tr>
                                    <th>{{ Str::headline('akun penyusutan') }}</th>
                                    <td>{{ $model->depreciation_coa?->account_code }} - {{ $model->depreciation_coa?->name }}</td>
                                </tr>
                            </x-slot>
                        </x-table>
                    </x-slot>
        ...
        <div class="mt-20">
            <x-card-data-table title="{{ 'dokumen' }}">
                <x-slot name="table_content">
                    <x-table theadColor='danger'>
                        <x-slot name="table_head">
                            <th>#</th>
                        ...
                        <x-slot name="table_body">
                            @foreach ($model->leaseDocuments as $item)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ $item->name }}</td>
                                    <td>{{ localDate($item->transaction_date) }}</td>
                                    <td>{{ localDate($item->effective_date) }}</td>
                                    <td>{{ localDate($item->end_date) }}</td>
                                    <td>{{ $item->due_date }}</td>
