...
@section('content')
    @can('create purchase-general')
        <form action="{{ route('admin.purchase-order-general.store') }}" method="post" id="create-form" enctype="multipart/form-data">
            ...
            <x-card-data-table title="Pilih Purchase Request">
                <x-slot name="table_content">
                    <div class="row">
                        <div class="col-md-12 text-right align-self-end mb-2">
                            <x-button type="button" color="primary" label="PR Outstanding" data-toggle="modal" data-target="#pr-outstanding-modal" />
                        </div>
                        <div class="col-md-12" id="purchase-request-detail-list">
    ...
    @endcan
    <div class="modal fade" id="pr-outstanding-modal" tabindex="-1" role="dialog" aria-labelledby="pr-outstanding-label" aria-hidden="true">
        <div class="modal-dialog modal-xl" role="document">
            <div class="modal-content">
                <!-- Modal Header -->
                <div class="modal-header">
                    <h5 class="modal-title" id="pr-outstanding-label">PR Outstanding</h5>
                ...
                <div class="modal-body">
                    <x-table id="pr-outstanding-table">
                        <x-slot name="table_head">
                            <th>#</th>
                            <th>Tanggal</th>
                            <th>No. PR</th>
                ...
                <div class="modal-footer text-end">
                    ...
                    <x-button type="button" color="primary" label="Save" icon="save" fontawesome id="btn-submit" onclick="get_selected_purchase_requests()" />
                </div>
            </div>
        </div>
    </div>
...
@section('js')
    ...
    <script>
        ...
        function get_selected_purchase_requests() {
            $('#pr-outstanding-modal').modal('hide');

            $.ajax({
                url: "{{ route('admin.purchase-order-general.get-selected-purchase-request') }}",
                type: "post",
                data: {
                    _token: "{{ csrf_token() }}",
                ...
                success: function(res) {
                    purchaseRequests = res;
                    $('#purchase-request-detail-list').html('');
                    purchaseRequests.map((pr, purchase_request_index) => {
                        pr.children.map((purchase_request, row_index) => {
                            ...
                            $(`#purchase-request-detail-list`).append(`
                                    <div class="row">
                                        ${item_html}
                                        <div class="col-md-2">
                                            <div class="form-group">
                                        ...
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                <x-input type="text" name="main_price[]" label="harga" value="${purchase_request.price}" class="commas-form" id="main-item-price-${row_index}-${purchase_request_index}" required readonly />
                                            </div>
                                        </div>
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                <x-input type="text" name="main_jumlah_diminta[]" label="jumlah diminta" value="${formatRupiahWithDecimal(data.jumlah_diapprove)} / ${data.qty_po} / ${data.qty_lock} - ${data.item_data.unit.name}" helpers="Qty Dipesan / Qty Dibelikan / Qty di lock" id="main-item-quantityRequested-${row_index}-${purchase_request_index}" required readonly/>
                                            </div>
                                        </div>
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                <x-input type="text" name="main_quantity[]" label="jumlah" value="${formatRupiahWithDecimal(purchase_request.quantity)}" class="commas-form main-quantity" id="main-item-quantity-${row_index}-${purchase_request_index}" useCustomError required data />
                                            </div>
                                        </div>
                                                ...
                                                <x-modal title="Detail purchase request item" id="modalpurchase-requestDetail-modal-${row_index}-${purchase_request_index}" headerColor="info" modalSize="1000">
                                                    <x-slot name="modal_body">
                                                        <x-table id="">
                                                            ...
                                                            <x-slot name="table_body">
                                                                <tr>
                                                                    <td>${item_value} - ${data.item_data.unit?.name}</td>
                            ...
                            $(`#main-item-price-before-discount-${row_index}-${purchase_request_index}`).on('blur', function(e) {
                                ...
                                console.log(purchase_request.price_before_discount);
                            });
                                ...
                                if (this.value) {
                                    ...
                                    $.ajax({
                                        ...
                                        success: function({
                                            ...
                                            $(`#main-item-unit-${row_index}-${purchase_request_index}`).val(data.item_data.unit.name);
                                        }
                                    });
                                } else {
                            ...
                            $(`#main-item-quantity-${row_index}-${purchase_request_index}`).data('testQty', parseFloat(data.jumlah_diapprove - data.qty_po - data.qty_lock).toFixed(2));
                            $(`#main-item-quantity-${row_index}-${purchase_request_index}`).keyup(function(e) {
                                e.preventDefault();

                                let req_qty = parseFloat($(this).data('testQty')).toFixed(2);
                                purchase_request.quantity = thousandToFloat(this.value).toFixed(2);

                                if (parseFloat(purchase_request.quantity) > parseFloat(req_qty)) {
                                    ...
                                    // $(this).val(data.jumlah_diapprove - data.qty_po - data.qty_lock);
                                } else {
                                    ...
                                    if (this.value) {
                                        ...
                                        $.ajax({
                                            ...
                                            success: function({
                                                ...
                                                $(`#additional-item-unit-${row_index}`).val(data.item_data.unit.name);
                                            }
                                        });
                                    } else {
                                                ...
                                                return {
                                                    ...
                                                    purchase_request_detail_id: children.map((purchase_request_detail, purchase_request_detail_index) => {
                                                        return {
                                                            ...
                                                            quantity: purchase_request_detail.quantity,
                                                            price_before_discount: purchase_request_detail.price_before_discount,
                                                            discount: purchase_request_detail.discount,
                                                            price: purchase_request_detail.price,
                                                            tax_id: TAX_LIST_ID
                                                        };
                                                    })
                                                }
        ...
        $('#pr-outstanding-table').DataTable({
            processing: true,
            serverSide: true,
            responsive: true,
            destroy: true,
            order: [4, 'desc'],
            ajax: {
                url: '{{ route('admin.purchase-order-general.pr-outstanding-data') }}',
                type: 'POST',
                data: {
                    _token: token,
            ...
            columns: [{
                ...
                {
                    data: 'tanggal',
                    name: 'purchase_requests.tanggal'
                },
                {
                    data: 'kode',
                    name: 'purchase_requests.kode'
                },
                {
                    data: 'item_name',
                    name: 'items.nama'
                ...
                {
                    data: 'jumlah',
                    name: 'purchase_request_details.jumlah'
                },
                {
                    data: 'jumlah_diapprove',
                    name: 'purchase_request_details.jumlah_diapprove'
                },
                {
                    data: 'po_amount',
                    orderable: false,
