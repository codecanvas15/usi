    ...
    <div class="mt-2">
        ...
        <x-table theadColor="white" class="table-bordered mt-20">
            <x-slot name="table_head">
                @include('admin.sale-order-general-report.per-periode-sale-order-general-2.table.header')
            </x-slot>
            <x-slot name="table_body">
                @php
                    $no = 1;
                @endphp
                @include('admin.sale-order-general-report.per-periode-sale-order-general-2.table.body', [
                    'formatNumber' => true,
                ])
            </x-slot>
            <x-slot name="table_foot">
                @include('admin.sale-order-general-report.per-periode-sale-order-general-2.table.footer', [
                    'formatNumber' => true,
                ])
            </x-slot>
        </x-table>
