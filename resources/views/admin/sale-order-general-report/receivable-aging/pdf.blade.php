    ...
    <div class="mt-2">
        <div class="row">
            <div class="text-center">
                ...
                <p class="font-small-2 text-uppercase my-0">sampai tanggal : {{ localDate($to_date) }}</p>
            </div>
        </div>
        <br>
        <x-table theadColor="white" class="table-bordered mt-20">
            <x-slot name="table_head">
                @include('admin.sale-order-general-report.receivable-aging.table.header')
            </x-slot>
            <x-slot name="table_body">
                @include('admin.sale-order-general-report.receivable-aging.table.body', [
                    'formatNumber' => true,
                ])
            </x-slot>
            <x-slot name="table_foot">
                @include('admin.sale-order-general-report.receivable-aging.table.footer', [
                    'formatNumber' => true,
                ])
            </x-slot>
        </x-table>
