        ...
        <table style="width: 100%;">
            <tr>
                <td style="width: 75%; vertical-align: top">
                    <h2 class="text-uppercase my-0">Purchase Request</h2>
                    <span class="doc-number-border font-small-3 text-bold">NO. {{ $model->kode }}</span>
                </td>
            </tr>
        </table>
    </div>
        ...
        <tr>
            <td class="p-0 valign-top">
                <table class="small-font">
                    <tbody>
                        <tr>
                            <td class=" valign-top p-0" width="15%">Tanggal</td>
                            <td class=" valign-top p-0" width="2%">:</td>
                            <td class=" valign-top p-0">{{ localDate($model->tanggal) }}</td>
                        </tr>
                        <tr>
                            <td class=" valign-top p-0">Divisi</td>
                            <td class=" valign-top p-0">:</td>
                            <td class=" valign-top p-0 text-uppercase">{{ $model->division?->name }}</td>
                        </tr>
                        <tr>
                            <td class=" valign-top p-0">Project</td>
                            <td class=" valign-top p-0">:</td>
                            <td class=" valign-top p-0 text-uppercase">{{ $model->project?->name }}</td>
                        </tr>
                    </tbody>
                </table>
            </td>
        ...
        <div class="row" style="max-width: 100%">
            <table width="100%" style="margin-top: 5px;" class="table table-bordered">
                <tr style="background-color: black; color:white; height: 50px;">
                    ...
                    <th style="text-align: center; width: 16.6%; padding: 8px;border: 1px solid black;" class="text-semi-bold">Desc</th>
                </tr>
                @foreach ($model->purchase_request_details as $data)
                    <tr style="color:white; height: 50px;">
                        <td style="text-align: left; padding: 8px 20px;color:black;">{{ $data->item_id ? $data->item_data->nama : $data->item }}</td>
                        <td style="text-align: center; padding: 8px 20px;color:black;">{{ formatNumber($data->jumlah_diapprove) }}</td>
                        <td style="text-align: center; padding: 8px 20px;color:black;">{{ $data->unit?->name }}</td>
                        <td style="text-align: center; padding: 8px 20px;color:black;">{{ $data->keterangan }}</td>
                    </tr>
                @endforeach
            </table>
        </div>
