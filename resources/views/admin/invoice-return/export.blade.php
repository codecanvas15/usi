...
<head>
    <title>RETUR PENJUALAN - {{ $model->code }}</title>
    <style type="text/css">
        body {
                        ...
                        <table>
                            <tbody>
                                ...
                                <tr>
                                    <td>No. DO</td>
                                    <td>:</td>
                                    <td> {{ $model->reference_data?->code ?? ($model->reference_data->code ?? '') }}</td>
                                </tr>
                                <tr>
                                    <td>Gudang</td>
                                    <td>:</td>
        ...
        <div class="row mt-2">
            <table class="table table-bordered">
                <tr>
                    ...
                    <th width="3%"><b>Satuan</b></th>
                    <th width="3%"><b>Qty DO</b></th>
                    <th width="3%"><b>Qty</b></th>
                    <th width="5%"><b>Harga</b></th>
                    <th width="5%"><b>Sub total</b></th>
                    <th width="5%"><b>Total pajak</b></th>
                    <th width="5%"><b>Total</b></th>
                </tr>
                @foreach ($model->invoice_return_details as $item)
                    <tr>
                        <td>{{ $item->item?->nama }} - {{ $item->item?->kode }}</td>
                        <td class="text-center">{{ $item->unit?->name }}</td>
                        <td class="text-right">{{ formatNumber($item->do_qty) }}</td>
                ...
                <tr>
                    <td colspan="5"></td>
                    <td class="text-right" style="font-weight: bolder">{{ $model->currency->simbol }} {{ formatNumber($model->invoice_return_details->sum('subtotal')) }}</td>
                    <td class="text-right" style="font-weight: bolder">{{ $model->currency->simbol }} {{ formatNumber($model->invoice_return_details->sum('tax_amount')) }}</td>
                    <td class="text-right" style="font-weight: bolder">{{ $model->currency->simbol }} {{ formatNumber($model->invoice_return_details->sum('total')) }}</td>
                </tr>
            </table>
