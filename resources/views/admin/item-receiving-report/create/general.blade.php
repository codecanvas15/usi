...
@php
    ...
    $title = 'Laporan Penerimaan barang general';
@endphp
...
@section('content')
    @can('create item-receiving-report-general')
        <form action="{{ route("admin.$main.store") }}" id="lpb_general_submit" method="post" enctype="multipart/form-data">
            @csrf
            ...
            <x-card-data-table title="{{ $title }}">
                <x-slot name="table_content">
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <x-select name="purchase_order_general_id" label="Purchase Order General" required id="purchaseOrderGeneral-select"></x-select>
                                <div id="paymentStatus" class="badge badge-pill badge-danger animate__animated animate__pulse animate__infinite infinite" style="display: none">Uang Muka Telah dibayar!</div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <x-input type="text" name="do_code_external" label="Kode Do External" id="" />
                            </div>
                        </div>
                    </div>
            ...
            <x-card-data-table>
                <x-slot name="table_content">
                    <div class="d-flex justify-content-end gap-3" id="button_submit">
            ...
            $(document).ready(function() {
                ...
                const initialize = () => {
                    ...
                    const initSelectForm = () => {
                        const selectPurchaseOrder = () => {
                            ...
                            $(`select[name="#purchaseOrderGeneral-select"]`)
                                .toArray()
                                .map(function() {
                                    if ($(this).val() != null) {
                            ...
                            let target_value = $(`#purchaseOrderGeneral-select`).val();
                            ...
                            var itemSelect = {
                                ...
                                ajax: {
                                    url: "{{ route('admin.purchase-order-general.select-for-receiving') }}",
                                    dataType: "json",
                                    delay: 250,
                                    type: "get",
                                    data: (params) => {
                                        ...
                                        result["purchaseOrderGeneral-select"] = target_value;
                            ...
                            $(`#purchaseOrderGeneral-select`).select2(itemSelect);
                            return;
                        };

                        initSelect2SearchPaginationData(`warehouse-select`, `{{ route('admin.select.ware-house.type') }}/general`, {
                            id: 'id',
                            text: 'nama'
                        })
                    ...
                    $('#branch_id').change(function(e) {
                        $('#purchaseOrderGeneral-select').val(null).trigger('change');
                    });

                    const handleOnchangePurchaseOrderForm = () => {
                        $('#purchaseOrderGeneral-select').change(function(e) {
                            e.preventDefault();
                            ...
                            if (value) {
                                $.ajax({
                                    type: "get",
                                    url: "{{ route('admin.purchase-order-general.detail-for-receiving') }}" + '/' + value,
                                    success: function({
                                        data
                                    }) {
                                        ...
                                        purchaseOrderGeneral = data.model;
                                        purchaseOrderGeneralDetails = data.details;
                                        ...
                                        $.ajax({
                                            url: '/item-receiving-report-general/' + value + '/get-payment-status-po-general',
                                            type: 'GET',
                                            success: function(res) {
                                                if (res.success) {
                                                    let data = res.data;
                                                    if (data.cash_advance_payment_id) {
                    ...
                    const displayItems = () => {
                        ...
                        var data_index = 0;
                        purchaseOrderGeneralDetails.map((parent_detail, parent_index) => {

                            parent_detail.purchase_order_general_detail_items.map((detail, index) => {
                                ...
                                let html = `<div class="row" id="row-data-${data_index}">
                                        ...
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                <x-input type="text" id="item_id-${data_index}" name="item" label="item" value="" required readonly />
                                            </div>
                                        </div>
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                <x-input type="text" id="jumlah-data-${data_index}" name="jumlah" label="quantity" value="${formatRupiahWithDecimal(detail.quantity_received)} / ${formatRupiahWithDecimal(detail.quantity)} - ${detail.unit?.name}" required helpers="sudah diterima / dipesan" readonly />
                                            </div>
                                        </div>
                                        <div class="col-md-3">
                                            <div class="form-group">
                                ...
                                $(`#item_id-${data_index}`).val(`${detail.item.nama} - ${detail.item.kode}`);
                ...
                const handleSubmit = (data_index = 0) => {
                    $(`#lpb_general_submit`).submit(function(e) {
                        ...
                        if ($(`#jumlah-diterima-${data_index}`).val() == "NaN") {
                            e.preventDefault()
