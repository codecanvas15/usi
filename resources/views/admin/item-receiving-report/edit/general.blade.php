            ...
            <x-card-data-table title="edit {{ $title }}">
                <x-slot name="table_content">
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <x-input name="vendor_id" label="purchase order" value="{{ $model->reference->code }}" required id="vendor-form" disabled />
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            const init = () => {
                initSelect2SearchPaginationData(`warehouse-select`, `{{ route('admin.select.ware-house') }}/general`, {
                    id: 'id',
                    text: 'nama'
                })
                getData();
            ...
            const getData = () => {
                $.ajax({
                    type: "GET",
                    url: "{{ route('admin.item-receiving-report-general.detail-for-edit-data', $model) }}",
                    success: function({
                        data
                    }) {
                ...
                ITEM_RECEIVING_REPORT.map((data, index) => {
                    let html = `
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    <x-input type="text" id="item_id-${index}" name="item" label="item" value="" required readonly />
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                    ...
                    $('#item_id-' + index).val(`${data.item.nama} - ${data.item.kode}`);
