function getLpbDetail(e) {
    ...
    $.ajax({
        type: "get",
        url: `${base_url}/purchase-return/${$(e).val()}/get-lpb-detail`,
        success: function (data) {
            var html = '';
            $('.currency_kode').text(data.data.currency.kode);
            $('#item_receiving_report_date').val(data.data.date_receive);
            ...
            $.each(data.data.details, function (index, value) {
                ...
                $.each(value.taxes, function (index_tax, value_tax) {
                    if (value_tax.tax == null) {
                        value_tax.tax = value_tax.tax_trading;
                    }
                    taxes += `<div class="mb-1">
                                ...
                                <input class="tax_trading_id_${index}" type="hidden" id="tax_trading_id_${index}_${index_tax}" name="tax_trading_id[${value.reference_id}][]" value="${value_tax.tax_trading_id}" data-index="${index_tax}">
                ...
                html += ` <tr id="row-${index}">
                                <td class="align-top">
                                    <input type="hidden" name="purchase_return_detail_id[]" value="">
                                ...
                                <td class="text-end align-top">
                                    <input type="text" id="lpb_qty_${index}" name="lpb_qty[]" value="${formatRupiahWithDecimal(value.qty)}" class="form-control text-end" readonly>
                                    <br>
                                    <span><b>QTY Retur/Terpakai :</b> ${formatRupiahWithDecimal(value.return_qty)}</span>
...
function resetForm() {
    $('#return-data').html('');
    $('#item_receiving_report_id').val('').trigger('change');
}
...
function check_unique_tax_number(except_id) {
    $.ajax({
        type: "post",
        url: `${base_url}/purchase-return/check-unique-tax-number`,
        data: {
            _token: token,
...
function validateDate() {
    ...
    let item_receiving_report_date = $('#item_receiving_report_date').val();
    item_receiving_report_date = item_receiving_report_date.split('-').reverse().join('');

    if (date != '' && item_receiving_report_date != '') {
        if (date < item_receiving_report_date) {
            showAlert('', 'Tanggal retur tidak boleh lebih kecil dari tanggal LPB', 'warning');
            $('#date').val('');
