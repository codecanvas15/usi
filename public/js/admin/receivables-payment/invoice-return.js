...
function getReturnSelect() {
    var except_id = [];
    $("input[name='invoice_return_id[]']").each(function () {
    ...
    $.ajax({
        type: "post",
        url: `${base_url}/receivables-payment/invoice-return-select`,
        data: {
            _token: token,
            ...
            except_id: except_id,
            receivables_payment_id: $("#receivables_payment_id").val(),
            date: $("#date").val()
        },
            ...
            $.each(data, function (index, value) {
                if (value.outstanding != 0) {
                    var html = `<tr>
                                    ...
                                    <td>
                                        <input type="checkboxs" class="ichack-input" name="check_invoice_return_id[]" value="${value.id}" id="invoice_return_id${value.id}">
                                    </td>
                                </tr>`;
...
function saveSelectedReturn() {
    var array = [];
    $("input:checkbox[name='check_invoice_return_id[]']:checked").each(
        function () {
            array.push($(this).val());
        }
    );
            ...
            $.each(data, function (index, value) {
                var html = `<tr id="selected_return_row_${value.id}">
                                <td>
                                    ...
                                    <input type="hidden" name="receivables_payment_invoice_return_id[]" value="">
                                    <input type="hidden" name="invoice_return_id[]" value="${value.id}">
                                    <input type="hidden" name="return_total[]" id="return_total_${value.id}" value="${value.total}">
                                </td>
                                <td>${value.code}</td>
                                ...
                                <td>
                                    <button type="button" class="btn btn-sm btn-danger" onclick="$('#selected_return_row_${value.id}').remove();calculate_return_total();calculate_final_total()">
...
function calculate_row_return(index, is_foreign = false) {
    ...
    let row_return_total = $("#return_total_" + index);
    ...
    var gap = row_return_total * row_return_exchange_rate - row_return_total * exchange_rate;
